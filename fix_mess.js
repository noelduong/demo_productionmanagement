...
// 1. Define clean nplTab content
const cleanNplTab = \`    <!-- TAB: NPL & APPROVAL -->
...
// 2. Regex to match the messy block between NPL tab and History tab
const messyRegex = /<!-- TAB: NPL & APPROVAL -->[\\s\\S]*?<div id="historyTab"/;
if (html.match(messyRegex)) {
    html = html.replace(messyRegex, cleanNplTab + '\\n\\n    <!-- TAB: HISTORY -->\\n    <div id="historyTab"');
    console.log("Messy block between NPL and History cleaned up!");
} else {
    // Try matching without the comment
    const messyRegex2 = /<div id="nplTab"[\\s\\S]*?<div id="historyTab"/;
    if (html.match(messyRegex2)) {
        html = html.replace(messyRegex2, cleanNplTab + '\\n\\n    <!-- TAB: HISTORY -->\\n    <div id="historyTab"');
        console.log("Messy block (v2) cleaned up!");
    }
}
