 ...
 * Xóa sạch + import 86 đơn đầu tiên.
 */
function importPart1() {
  ...
  SpreadsheetApp.getUi().alert("Part 1 xong! Details: " + d.length + " dòng, Orders: " + o.length + " dòng. Tiếp tục chạy importPart2()");
}

/**
 * BƯỚC 2: Chạy hàm này SAU khi importPart1() hoàn thành.
 * Append thêm 85 đơn còn lại.
 */
function importPart2() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  
  var detailSheet = ss.getSheetByName("data_order_details");
  var lastDetailRow = detailSheet.getLastRow();
  ...
  var nc = 21;
  d = d.map(function(r){var row=r.slice(0,nc);while(row.length<nc)row.push("");return row;});
  if(d.length>0) detailSheet.getRange(lastDetailRow+1,1,d.length,nc).setValues(d);
  
  var orderSheet = ss.getSheetByName("data_order");
  var lastOrderRow = orderSheet.getLastRow();
  ...
  var noc = 24;
  o = o.map(function(r){var row=r.slice(0,noc);while(row.length<noc)row.push("");return row;});
  if(o.length>0) orderSheet.getRange(lastOrderRow+1,1,o.length,noc).setValues(o);
  
  SpreadsheetApp.getUi().alert("Import hoàn tất! Thêm " + d.length + " details + " + o.length + " orders. Tổng: 171 đơn hàng.");
}
